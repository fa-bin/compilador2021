...
Struct_Token Scanner(FILE *arq) {
	...
	while (condicaoWhile == 0) {
		...
		if (isalpha(caractere) != 0 || caractere == '_') {  // se caractere for letra, eh !=0 ou pode comeï¿½ar com _ (underline)
			...
			lexm.lexema[i] = '\0';
			if (!feof(arq)) {
				fseek(arq, -1, SEEK_CUR); // quando tem 2 tokens colados um no outro. para processar o 2ï¿½
			}
			...
			else {  // nï¿½o ï¿½ num float, entï¿½o ï¿½ int
				fseek(arq, -1, SEEK_CUR); // ele estava pulando uma posiï¿½ï¿½o na leitura do caractere. fseek volta 1 pos
				lexm.tipo = Tipo_int;
				lexm.lexema[i] = '\0';
				return lexm;
			}
		...
		else if (caractere == '.') { // se comeï¿½ar com um ponto, obrigatoriamente o resto tem q ser numero float. se nao, erro.
			lexm.lexema[i] = caractere;
			i++;
			coluna++;
			caractere = getc(arq);
		...
		else if (caractere == '=') { // verifica se ï¿½ igualdade ou atribuiï¿½ï¿½o
			lexm.lexema[i] = caractere;
			i++;
			coluna++;
			caractere = getc(arq);
		...
		else if (caractere == '>') { // verifica se ï¿½ > ou >=
			lexm.lexema[i] = caractere;
			i++;
			coluna++;
			caractere = getc(arq);
		...
		//comentario
			...
			if (caractere == '/') { // comentario de linha 
				...
				while (loop_asterisco == 0) {
					...
					if (caractere == '*') { // testa o * do fim do comentario multilinha
						coluna++;
						caractere = getc(arq);
						if (caractere == '/') { // ver se o prox caractere ï¿½ um / pra finalizar o comentario
							coluna++;
							i++;
			...
			else { // se nï¿½o for nenhum dos casos acima, ï¿½ divisï¿½o
				lexm.lexema[i] = '/';
				i++;
			...
			coluna++;
			caractere = getc(arq);
			condicaoWhile = 0;  // volta para o inicio da funï¿½ï¿½o para verificar os proximos caracteres
		}
